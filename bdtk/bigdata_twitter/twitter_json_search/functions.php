<?php
	...
	function searchTwitterCURL($search, $page=1, $num=100, $inc=0){
		...
		$search_url = "http://search.twitter.com/search.json?q=" . urlencode($query) . "&rpp=100";
	...
	function searchGeoTwitterCURL($lat, $lon, $rad, $unit, $num=100, $page=1, $inc=0){
		...
		$search_url = "http://search.twitter.com/search.json?geocode=" . urlencode($query) . "&rpp=$num";
	...
	function searchGeoTwitterWithQueryCURL($lat, $lon, $rad, $search, $num=100, $page=1, $inc=0){
		...
		$search_url = "http://search.twitter.com/search.json?q=" . urlencode($query) . "&geocode=" . urlencode($latquery) . "&rpp=$num";
	...
	function searchGeoTwitterWithQueryUnitCURL($lat, $lon, $rad, $unit, $search, $num=100, $page=1, $inc=0){
		...
		//Strip any Hash Characters or + (change + to space)
		$query = str_replace("#", "", $query);
		$query = str_replace("+", " ", $query);
	...
	function writeToDB($item, $table, $db){
		...
		$dateT = date("Y-m-d H:i:s",strtotime($item['published']));


		$sql = "INSERT INTO `".$table."`(twitterPost, twitterID, dateT, name, link, upage, twittergeo, lang, profile) VALUES('$post','".stripIDTag($item[id])."','$dateT','$name','$item[link]','$item[author_uri]', '".$item[twitter]['geo']."', '".$item[twitter]['lang']."','".$item['link_image']."')";
        ...
        function writeJSONToDB($item, $table, $db){
                $post = mysql_escape_string(trim(strip_tags($item['text'])));
                $name = mysql_escape_string(trim(strip_tags($item['from_user'])));
		$dateT = date("Y-m-d H:i:s", strtotime($item['created_at']));
		$geo = $item["geo"]["coordinates"];

		if(is_array($geo)){
			$geo = $item["geo"]["coordinates"][0].", ". $item["geo"]["coordinates"][1];
		}else{
			$geo = "";
		}

                $sql = "INSERT INTO `".$table."`(twitterPost, twitterID, dateT, name, link, upage, twittergeo, lang, profile, source, from_user, from_user_id, from_user_name, to_user, to_user_id, to_user_name) VALUES('$post','".$item["id_str"]."','$dateT','$name','$item[profile_image_url]','http://www.twitter.com/$name', '".$geo."', '".$item["iso_language_code"]."','".$item['profile_image_url']."', '".$item[source]."', '".$item[from_user]."', '".$item[from_user_id]."','".mysql_escape_string(trim(strip_tags($item[from_user_name])))."', '".$item[to_user]."', '".$item[to_user_id]."', '".mysql_escape_string(trim(strip_tags($item[to_user_name])))."')";
	...
	function writeJSONGeoItemToDB($item, $table, $db){

		$post = mysql_escape_string(trim(strip_tags($item->text)));
		$name = mysql_escape_string(trim(strip_tags($item->from_user)));
		$atom_content = mysql_escape_string(trim(strip_tags($item->text)));
		$g_location = mysql_escape_string(trim(strip_tags($item->location)));
		$source = mysql_escape_string(trim(strip_tags($item->source)));

		$geopoint = "";

		$twitterURl = "http://twitter.com/".$name."/statuses/".$item->id."";
		$authorURL = "http://twitter.com/".$name."/";
		$dateT = date("Y-m-d H:i:s", strtotime($item->created_at));

		if((sizeof($item->geo)) != 0){
			//Try GeoRSS
			if($item->geo->coordinates[0] != ""){
				$geopoint = $item->geo->coordinates[0].",".$item->geo->coordinates[1];
			}
		}else{
			$geopoint = "";
		}

		$sql = "INSERT INTO `".$table."`(twitterPost, twitterID, dateT, name, link, upage, twittergeo, lang, profile, google_location, atom_content, source) VALUES('$post','". $item->id."','".$dateT."','$name','$twitterURl','$authorURL', '".$geopoint."', '".$item->iso_language_code."','".$item->profile_image_url."', '".$g_location."', '".$atom_content."', '".$source."')";
	...
	function isItInDB($twitterID, $table, $db){
	...
	function createTwitterTableStore($tableName, $tableComment, $db){
		...
		$createTableSql = "CREATE TABLE IF NOT EXISTS `$tableName` (
  								...
  								`source` varchar(255) DEFAULT NULL,
  								 INDEX (twitterID)
							) ENGINE=MyISAM  DEFAULT CHARSET=utf8 DEFAULT CHARACTER SET utf8 DEFAULT COLLATE utf8_general_ci COMMENT='".$tableComment."'";
	...
	function createJSONTwitterTableStore($tableName, $tableComment, $db){
                global $db_name;
                $createTableSql = "CREATE TABLE IF NOT EXISTS `$tableName` (
                                                                `twitterID` varchar(255) NOT NULL,
                                                                `twitterPost` longtext NOT NULL,
	...
	function createGeoTwitterTableStore($tableName, $tableComment, $db){
		...
		$createTableSql = "CREATE TABLE IF NOT EXISTS `$tableName` (
  								...
  								`source` varchar(255) DEFAULT NULL,
  								 INDEX (twitterID)
  							) ENGINE=MyISAM DEFAULT CHARACTER SET utf8 DEFAULT COLLATE utf8_general_ci DEFAULT CHARSET=utf8 COMMENT='".$tableComment."'";
		$res = mysql_query($createTableSql, $db);
	...
	function saveAndTruncate($tempTablename,  $db){
		...
		if($res){
			(int)$truncateNum = mysql_num_rows($res) - $maxNumtoLeave;

			if($truncateNum > 0){
			$sql = "SELECT * FROM `$tempTablename` group by twitterID ORDER BY dateT asc LIMIT $truncateNum ";

			$copySQL = "INSERT INTO `$masterTable` ( twitterID, twitterPost,  dateT, `name`, link, upage, twittergeo, lang, profile, source, from_user, from_user_id, from_user_name, to_user, to_user_id, to_user_name) $sql";
