<?php 
	...
	function writeLine($fh, $id, $location, $db){
		...
		$sql = "SELECT * FROM $table WHERE twitterID = $id";
	...
	$db = mysql_connect("localhost", "bigdata1", "bigdata1") or die("Failed to Connect to DB!\n");
	mysql_select_db("bigdata", $db) or die("Failed to find Table!\n");

	$sql = "SELECT twitterID, twittergeo, google_location FROM `$table` Order by twitterID";
	$res = mysql_query($sql, $db);

	if(!$res)
		die("Query has a problem!:  \n\t". $sql."\n");
	...
	while ($c = mysql_fetch_assoc($res)){
		if($c['twittergeo'] === ''){
			$strippedLocation = stripPunc(stripAllAlphabet($c['google_location']));
			$arr = matchedLatLon($strippedLocation);

			if($arr != false) {
				//Matched a Lat Lon
				//print_r($c);
				writeLine($fh, $c['twitterID'], $arr, $db);
				$count++;
			}
		}else{
			writeLine($fh, $c['twitterID'], $c['twittergeo'], $db);
			$count++;
		}
